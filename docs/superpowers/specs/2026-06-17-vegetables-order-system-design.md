...
- 小程序端：微信登录、首页选菜、地址管理、订单列表、会员卡、我的。
...
小程序使用 Taro + NutUI。第一版保留底部 Tab，但结构比参考 HTML 更完整。底部 Tab 固定为：首页、订单、会员卡、我的，不设置单独“预订”菜单；预订能力放在首页。
...
- 底部导航只有：首页、订单、会员卡、我的；预订不是独立 Tab。
- 预订页参考餐饮点单页的信息效率：左侧分类固定，右侧菜品列表连续操作，底部固定已选篮子；但视觉保持蔬菜生鲜主题，不做咖啡促销式强营销。
- 我的页和会员卡页参考会员产品的身份卡片感，突出当前套餐、剩余额度、今日预订和常用服务。
...
- 小程序：首页选菜 -> 提交确认 -> 修改已预订内容 -> 订单列表 -> 地址管理 -> 会员卡 -> 我的。
...
- 小程序至少包含：首页预订、首页无套餐、提交与修改确认、修改已预订内容、订单列表、地址管理、会员卡、我的。
- 使用真实业务字段和模拟数据。
